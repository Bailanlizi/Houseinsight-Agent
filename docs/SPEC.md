...
3. **`search_listings` v0.1 行为**：采用「LLM 将自然语言解析为结构化筛选条件 → 校验后调用 `filter_rows` / 等价逻辑」；不做向量检索与全文搜索引擎（可作为后续增强）。  
...
- FastAPI：**上传 CSV、启动一次分析、查询会话状态/日志**（WebSocket 可选简化）。  
...
事件：`node_enter`, `node_exit`, `tool_call`, `tool_result`, `final`, `error`
